  ...
  class FourteenTwo {
    ...
    public static char[][] buildCave(List<List<int[]>> rockPaths) {
      foreach (var path in rockPaths) {
        foreach (var plot in path) {
          ...
          maxDepth = Math.Max(maxDepth, plot[0] + 3);
          maxWidth = Math.Max(maxWidth, plot[1] + 1000);
        }
      }
      ...
      char[] floor = new char[maxWidth];
      for (var i = 0; i < maxWidth; i++) floor[i] = '#';
      cave[maxDepth - 1] = floor;
    ...
    private static void Print(char[][] cave) {
      ...
      for (var i = 0; i < maxDepth; i++) {
        var row = cave[i];
        for (var j = minWidth - 10; j < cave[0].Length; j++) {
